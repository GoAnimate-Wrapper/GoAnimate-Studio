package anifire.component.widgets
{
   ...
   public class GroupHBarChartPart extends WidgetPart
   {
      ...
      public static const BAR_ANIMATION_OVERSHOOT:Number = 1.6;
      
      private static const GROUP_GAP:Number = 3;
      ...
      public function GroupHBarChartPart()
      {
         super();
      }
      ...
      override public function draw() : void
      {
         ...
         this._valueAxisOffset = _bounds.width * (-_widgetMaker.negativeMaxValue / _loc4_);
         var _loc8_:Number = GROUP_GAP;
         if(_numRanges == 1)
         {
            _loc8_ = 0;
         }
         this._currentBarHeight = _bounds.height - _loc8_ * 2;
         var _loc9_:int = 0;
         while(_loc9_ < _numRanges)
         {
            ...
            else if(_values[_loc9_])
            {
               _loc3_ = true;
            }
            this._barRatio = _values[_loc9_] / _loc4_;
            this._currentBarWidth = _bounds.width * this._barRatio * _widgetMaker.easeOutBack(this._rangeAnimationFactor[_loc9_],BAR_ANIMATION_OVERSHOOT);
            _loc10_ = MIN_WIDTH;
            if(_loc7_ > 0)
            {
               _loc10_ = _loc10_ + (_loc6_ + _loc7_) * 2;
            }
            this._barRect.height = this._currentBarHeight / _numRanges;
            this._barRect.y = _bounds.y + (_bounds.height - this._currentBarHeight) * 0.5 + _loc9_ * this._barRect.height + _loc8_;
            if(this._currentBarWidth >= 0)
            {
               if(this._currentBarWidth < _loc10_)
               {
                  this._currentBarWidth = _loc10_;
               }
            ...
            if(_loc13_ || _loc11_)
            {
               ...
               if(_loc13_)
               {
                  ...
                  if(this._currentBarWidth > 0)
                  {
                     this._labelRect[_loc9_].x = this._barRect.x + this._barRect.width + WidgetConstants.LABEL_PADDING * 2;
                     if(_loc7_ > 0)
                     {
                        this._labelRect[_loc9_].x = this._labelRect[_loc9_].x + WidgetConstants.LABEL_PADDING * 2;
                     }
                  }
                  else
                  {
                     this._labelRect[_loc9_].x = this._barRect.x - WidgetConstants.LABEL_PADDING * 2;
                     if(_loc7_ > 0)
                     {
                        this._labelRect[_loc9_].x = this._labelRect[_loc9_].x - WidgetConstants.LABEL_PADDING * 2;
                     }
                  }
                  this._labelRect[_loc9_].y = this._barRect.y + this._barRect.height / 2;
            ...
            while(_loc2_ < _numRanges)
            {
               ...
               switch(_widgetMaker.getProperty("valuePosition"))
               {
                  case "left":
                     if(values[_loc2_] >= 0)
                     {
                        _loc3_.x = _loc1_ + WidgetConstants.LABEL_PADDING;
                     }
                     else
                     {
                        _loc3_.x = _loc1_ - _loc3_.width - WidgetConstants.LABEL_PADDING;
                     }
                     _loc3_.y = this._labelRect[_loc2_].y - _loc3_.height / 2;
                     if(_loc3_.height > this._labelRect[_loc2_].height || _loc3_.width + WidgetConstants.LABEL_PADDING > this._labelRect[_loc2_].width)
                     {
                  ...
                  default:
                     ...
                     if(values[_loc2_] >= 0)
                     {
                        _loc3_.x = this._labelRect[_loc2_].x;
                     }
                     else
                     {
                        _loc3_.x = this._labelRect[_loc2_].x - _loc3_.width;
                     }
                     if(_loc3_.height > this._labelRect[_loc2_].height)
                     {
                        _loc3_.visible = false;
                     }
                     else
